...
- `thread/setPinnedPrompt` — set or clear (`null`) a thread's pinned prompt text.
- `thread/exportMarkdown` — export a thread rollout to markdown and optionally persist it to `outputPath`.
- `thread/archive` â€” move a threadâ€™s rollout file into the archived directory; returns `{}` on success.
...
- `thread/unarchive` â€” move an archived rollout file back into the sessions directory; returns the restored `thread` on success.
...
- `turn/start` â€” add user input to a thread and begin Codex generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode".
...
- `agent/interrupt` — request interruption of the active agent in a thread; returns `{}` when accepted.
- `hub/status` — read local hub daemon status (endpoint, LAN mode, paired device count).
- `hub/configureLan` — enable or disable LAN companion mode and set LAN bind host/port.
- `pair/start` — mint a short-lived pairing code for companion clients.
- `pair/complete` — complete pairing with a valid code and return a long-lived device token.
- `device/list` — list paired devices (cursor + limit pagination).
- `device/revoke` — revoke a paired device by `deviceId`.
- `capture/screenshot` — capture a local screenshot and return the saved path.
- `asset/uploadClipboardImage` — persist a clipboard image payload (`imageBase64`) into local assets.
...
- `model/list` â€” list available models (with reasoning effort options and optional `upgrade` model ids).
- `experimentalFeature/list` â€” list feature flags with stage metadata (`beta`, `underDevelopment`, `stable`, etc.), enabled/default-enabled state, and cursor pagination. For non-beta flags, `displayName`/`description`/`announcement` are `null`.
- `collaborationMode/list` â€” list available collaboration mode presets (experimental, no pagination).
- `skills/list` â€” list skills for one or more `cwd` values (optional `forceReload`).
- `skills/remote/read` â€” list public remote skills (**under development; do not call from production clients yet**).
- `skills/remote/write` â€” download a public remote skill by `hazelnutId`; `isPreload=true` writes to `.codex/vendor_imports/skills` under `codex_home` (**under development; do not call from production clients yet**).
- `app/list` â€” list available apps.
- `skills/config/write` â€” write user-level skill config by path.
...
- `mcpServerStatus/list` â€” enumerate configured MCP servers with their tools, resources, resource templates, and auth status; supports cursor+limit pagination.
- `feedback/upload` â€” submit a feedback report (classification + optional reason/logs and conversation_id); returns the tracking thread id.
- `command/exec` â€” run a single command under the server sandbox without starting a thread/turn (handy for utilities and validation).
- `config/read` â€” fetch the effective config on disk after resolving config layering.
- `config/value/write` â€” write a single config key/value to the user's config.toml on disk.
- `config/batchWrite` â€” apply multiple config edits atomically to the user's config.toml on disk.
- `configRequirements/read` â€” fetch loaded requirements constraints from `requirements.toml` and/or MDM (or `null` if none are configured), including allow-lists (`allowedApprovalPolicies`, `allowedSandboxModes`, `allowedWebSearchModes`), `enforceResidency`, and `network` constraints.
...
{ "id": 12, "result": { "thread": { "id": "thr_456", â€¦ } } }
{ "method": "thread/started", "params": { "thread": { â€¦ } } }
```
...
- `item/completed` â€” sends the final `item` once that work finishes (e.g., after a tool call or message completes); treat this as the authoritative state.
...
2. `item/commandExecution/requestApproval` (request) â€” carries the same `itemId`, `threadId`, `turnId`, optionally `reason`, plus `command`, `cwd`, and `commandActions` for friendly display.
3. Client response â€” `{ "decision": "accept", "acceptSettings": { "forSession": false } }` or `{ "decision": "decline" }`.
4. `item/completed` â€” final `commandExecution` item with `status: "completed" | "failed" | "declined"` and execution output. Render this as the authoritative result.
...
2. `item/fileChange/requestApproval` (request) â€” includes `itemId`, `threadId`, `turnId`, and an optional `reason`.
3. Client response â€” `{ "decision": "accept" }` or `{ "decision": "decline" }`.
4. `item/completed` â€” returns the same `fileChange` item with `status` updated to `completed`, `failed`, or `declined` after the patch attempt. Rely on this to show success/failure and finalize the diff state in your UI.
